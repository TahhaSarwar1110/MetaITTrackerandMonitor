...
class YOLOTracker:
    def __init__(self, config):
        ...
        self.first_seen = {}
        self.last_seen = {}
        self.track_classes = {}  # stores class name per track_id

        self.csv_file = "demo_log233.csv"
        self._setup_csv()
    ...
    def run(self):
        ...
        while True:
            ...
            tahha_results = self.tahha_model.track(
                frame, persist=True, conf=self.config["tahha_conf"], verbose=False
            )[0]

            # Tahha boxes
            tahha_boxes = []
            if tahha_results.boxes:
                for b in tahha_results.boxes.xyxy.cpu().numpy():
                    tahha_boxes.append((int(b[0]), int(b[1]), int(b[2]), int(b[3])))
            ...
            if base_results.boxes and base_results.boxes.id is not None:
                ...
                for box, track_id, cls_id in zip(boxes, track_ids, cls_ids):
                    ...
                    if track_id not in self.first_seen:
                        ...
                        self.track_classes[track_id] = final_cls_name  # store class identity
        ...
        for track_id in self.first_seen:
            if track_id in self.last_seen:
                ...
                cls_name = self.track_classes.get(track_id, "unknown")
                cls_id = 11 if cls_name == "tahha" else 0

                self._log(track_id, cls_id, cls_name, "EXIT", 0, 0, duration)
